...
export function selectPublication(
  ...
  if (config.publications.length === 0) {
    throw new Error(
      "No Substack credentials configured. Set SUBSTACK_PUBLICATION_URL and SUBSTACK_SESSION_TOKEN, or run `substack-mcp login`. See https://github.com/thenavidm/substack-mcp-cli#3-setup-",
    );
  }
