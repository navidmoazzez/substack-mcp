 ...
 *
 * The package ships two binaries onto the same file. Printing `substack-mcp` at
 * someone who typed `substack-cli` hands them a command that works but is not
 * the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("substack-cli") ? "substack-cli" : "substack-mcp";
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with SUBSTACK_READ_ONLY=1.`
